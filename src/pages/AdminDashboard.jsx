...
export default function AdminDashboard() {
  ...
  const handleStartVideoCall = async () => {
    if (!activeChatId) return;
    const roomName = `GlassChat-${activeChatId}-${Math.random().toString(36).substring(2, 9)}`;
    await addDoc(collection(db, 'chats', activeChatId, 'messages'), { text: "Started a Secure Video Call", isVideoCall: true, roomName: roomName, sender: "admin", timestamp: serverTimestamp(), status: "sent" });
  ...
  const handleSendMessage = async (e, overrideText = null) => {
    ...
    try {
      ...
      scrollToBottom('auto');
    } catch (err) { alert("Delivery Failed: ID may be blocked or expired."); } finally { setIsUploading(false); }
  };
                  ...
                  <>
                    <header className="flex-none shrink-0 bg-[#F9F6F0]/70 backdrop-blur-xl border-b border-[#C1B2A6]/50 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between z-20 shadow-[0_10px_30px_rgba(90,70,50,0.03)]" style={{ paddingTop: 'calc(0.75rem + env(safe-area-inset-top))' }}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={handleStartVideoCall} title="Start Secure Video Call" className="p-2 sm:p-2.5 rounded-xl bg-[#F9F6F0] border border-[#C1B2A6]/50 text-blue-600 hover:text-blue-800 shadow-sm"><Video size={18} /></button>
                                ...
                                <div className={`relative px-4 py-3 sm:px-5 sm:py-3.5 max-w-[85%] sm:max-w-[70%] rounded-2xl shadow-sm border ${isAdmin ? 'bg-[#5A4535] border-[#423226] text-[#F9F6F0] rounded-tr-sm' : 'bg-[#F9F6F0] border-[#C1B2A6]/50 text-[#4A3C31] rounded-tl-sm'}`}>
                                  {msg.isVideoCall ? (
                                    <div className="flex flex-col items-center justify-center p-3 bg-[#F9F6F0]/10 rounded-xl border border-[#C1B2A6]/30 mb-1">
                                      ...
                                      <button onClick={() => setActiveVideoRoom(msg.roomName)} className={`px-5 py-2 rounded-full font-bold text-xs shadow-md transition-transform hover:scale-105 ${isAdmin ? 'bg-[#F9F6F0] text-[#5A4535]' : 'bg-[#5A4535] text-[#F9F6F0]'}`}>
                                        JOIN IN-APP
                                      </button>
                                    </div>
                                  ) : msg.isImage ? (
                                  ...
                                  <div className={`text-[10px] font-medium mt-1.5 flex items-center gap-0.5 ${isAdmin ? 'justify-end text-[#C1B2A6]' : 'justify-start text-[#9E8E81]'}`}>
                                    {/* ✅ SILENT EDIT: Admin edits do not show "edited" tag to keep it professional */}
