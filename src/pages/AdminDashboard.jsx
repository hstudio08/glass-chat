...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribeMessages = onSnapshot(query(collection(db, 'chats', activeChatId, 'messages'), orderBy('timestamp', 'asc')), (snapshot) => {
      ...
      if (previousMessageCount.current !== 0 && fetchedMessages.length > previousMessageCount.current) {
        if (fetchedMessages[fetchedMessages.length - 1]?.sender === 'user') {
  ...
  const handleSendMessage = async (e, overrideText = null) => {
    ...
    setIsUploading(true); setNewMessage(""); setPendingImage(null); setPreviewUrl(null); setShowAIReplies(false); setReplyingToId(null);
    if (textareaRef.current) textareaRef.current.style.height = '48px';
    scrollToBottom('auto');
  ...
  if (!adminUser) {
    return (
      <div className="fixed inset-0 w-full h-[100dvh] flex items-center justify-center p-4 bg-gradient-to-br from-[#E6DCC8] to-[#D5C7B3] overflow-hidden font-sans text-[#4A3C31]">
  ...
  return (
    ...
    <div className="flex flex-col sm:flex-row w-full h-[100dvh] bg-[#F9F6F0] font-sans text-[#4A3C31] overflow-hidden relative">
      ...
      <AnimatePresence mode="wait">
                              ...
                              <div className={`relative px-4 py-3 sm:px-5 sm:py-3.5 max-w-[85%] sm:max-w-[70%] rounded-2xl shadow-sm border break-words overflow-hidden ${isAdmin ? 'bg-[#5A4535] border-[#423226] text-[#F9F6F0] rounded-tr-sm' : 'bg-[#F9F6F0] border-[#C1B2A6]/50 text-[#4A3C31] rounded-tl-sm'}`}>
                                {msg.isVideoCall ? (
                                  <div className="flex flex-col items-center justify-center p-3 bg-[#F9F6F0]/10 rounded-xl border border-[#C1B2A6]/30 mb-1">
                                    ...
                                    <button onClick={() => setVideoCallState({ roomId: msg.roomName, isIncoming: false })} className={`px-5 py-2 rounded-full font-bold text-xs shadow-md transition-transform hover:scale-105 ${isAdmin ? 'bg-[#F9F6F0] text-[#5A4535]' : 'bg-[#5A4535] text-[#F9F6F0]'}`}>
                                      JOIN ROOM
                                    </button>
                                  </div>
                                ) : msg.isImage ? (
                        ...
                        <div className="flex-1 relative bg-[#F9F6F0] border border-[#C1B2A6]/50 rounded-2xl shadow-sm focus-within:border-[#8C7462] focus-within:ring-2 focus-within:ring-[#E8E1D5] transition-all flex flex-col p-1.5">
                          ...
                          <div className="flex items-end w-full">
                            ...
                            <div className="absolute right-1.5 bottom-1.5">
                              <motion.button onClick={handleSendMessage} type="button" whileTap={{ scale: 0.95 }} disabled={(!newMessage.trim() && !pendingImage) || isOffline || isUploading} className={`p-2 rounded-xl text-[#F9F6F0] shadow-sm disabled:opacity-50 transition-opacity min-h-[36px] min-w-[36px] flex items-center justify-center ${editingMsgId || ghostMode ? 'bg-[#8C7462]' : 'bg-[#5A4535]'}`}>
        ...
        {/* --- SETTINGS --- */}
        {activeTab === 'settings' && (
          <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={animTween} className="flex-1 flex flex-col w-full h-full min-h-0 p-4 sm:p-8 overflow-y-auto" style={{ paddingTop: 'max(1.5rem, env(safe-area-inset-top))' }}>
            <div className="flex-none shrink-0 mb-6 border-b border-[#C1B2A6]/50 pb-4">
